...
import { deleteMedicineCategory, moveMedicineCategory } from "./actions";

export default async function AdminMedicineCategoryPage() {
  ...
  return (
    <div>
      <PageHead
        title="จัดการหมวดหมู่ยา"
        action={
          <Link href="/admin/medicine-category/create" className={btnPrimary}>
      ...
      <AdminCard className="overflow-x-auto">
        <table className="w-full text-sm">
          ...
          <tbody className="divide-y divide-border">
              ...
              <tr key={cat.id}>
                ...
                <td className="px-5 py-2.5">
                  <div className="flex justify-end gap-1.5">
                    <form action={moveMedicineCategory}>
                    ...
                    <Link href={`/admin/medicine-category/${cat.id}/edit`} className={btnGhostSm}>แก้ไข</Link>
                    <form action={deleteMedicineCategory}>
                      <input type="hidden" name="id" value={cat.id} />
            ...
            {categories.length === 0 && <tr><td colSpan={3} className="px-5 py-10 text-center text-text-muted">ยังไม่มีหมวดหมู่ยา</td></tr>}
          </tbody>
        </table>
      </AdminCard>
    </div>
