...
import { AdminCardHead } from "@/components/admin/admin-card";
import { btnGhostSm } from "@/components/admin/button-styles";
import {
  markAllMedicineNotificationsRead,
  markAllNotificationsRead,
  markMedicineNotificationRead,
  markNotificationRead,
} from "./actions";

export default async function AdminNotificationPage() {
  await requirePageAccess("notification");
  const [notifications, medicineNotifications] = await Promise.all([
    prisma.notification.findMany({ orderBy: [{ is_read: "asc" }, { created_at: "desc" }] }),
    prisma.medicineNotification.findMany({ orderBy: [{ is_read: "asc" }, { created_at: "desc" }] }),
  ]);
  const hasUnread = notifications.some((n) => !n.is_read);
  const hasUnreadMedicine = medicineNotifications.some((n) => !n.is_read);

  return (
    <div>
      <PageHead title="การแจ้งเตือน" />
      ...
      <div className="mb-6 flex flex-col gap-2">
        ...
        {notifications.length === 0 && <p className="py-6 text-center text-sm text-text-muted">ไม่มีการแจ้งเตือน</p>}
      </div>
      ...
      <AdminCardHead
        ...
        action={
          hasUnreadMedicine && (
            <form action={markAllMedicineNotificationsRead}>
              <button type="submit" className="rounded-[10px] border border-border px-4 py-2.5 text-sm font-semibold text-text hover:bg-bg-soft">
                ทำเครื่องหมายว่าอ่านแล้วทั้งหมด
              </button>
            </form>
      ...
      <div className="flex flex-col gap-2">
        {medicineNotifications.map((n) => (
          ...
          >
            ...
            {!n.is_read && (
              <form action={markMedicineNotificationRead}>
                <input type="hidden" name="id" value={n.id} />
                <button type="submit" className={btnGhostSm}>
                  อ่านแล้ว
                </button>
        ...
        {medicineNotifications.length === 0 && <p className="py-6 text-center text-sm text-text-muted">ไม่มีการแจ้งเตือน</p>}
      </div>
    </div>
  );
}
